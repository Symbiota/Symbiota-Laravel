<div
    x-data="{
        ...
        calculateSubMenuPosition(clickEvent) {
            ...
            for (let i = 0; i < submenus.length; i++) {
                ...
                if (
                    window.innerHeight <
                    submenus[i].previousElementSibling.getBoundingClientRect().top + submenus[i].offsetHeight
                ) {
    ...
    class="relative z-50 h-fit w-fit"
>
    ...
    <template x-teleport="#app-body">
        <div
            ...
            x-ref="contextmenu"
            class="text-base-content border-base-300 bg-base-100 fixed z-50 w-64 min-w-[8rem] rounded-md border p-1 text-sm shadow-md"
            x-cloak
        >
            @isset($menu)
                <div {{ $menu->attributes }}> {{ $menu }}</div>
            @endisset
    </template>
</div>

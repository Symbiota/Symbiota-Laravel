...
@endphp
<x-layout class="m-auto flex flex-col gap-4 p-0 sm:w-[95%] lg:w-[75%]">
    <h1 class="sr-only text-4xl font-bold">User Settings</h1>

    <div class="mt-4 flex items-center gap-2">
        <div
            class="border-base-300 bg-base-300 flex h-10 w-10 items-center justify-center rounded-full border font-bold"
        >
    ...
    >
        ...
        >
            <div class="flex items-center">
                ...
                <div class="flex flex-grow justify-end">
                    @can('CL_CREATE')
                        <x-button @click="show_create_form = true"> Create checklist </x-button>
                    @endcan
                </div>
            </div>
            <hr class="mb-4" />
            ...
            <form
                hx-post="{{ url('checklists/create') }}"
                hx-target="#user_checklists"
                hx-swap="outerHTML"
                x-show="show_create_form"
            >
                @csrf
                <fieldset class="flex flex-col gap-4">
                    <legend class="text-lg font-bold">Create New Checklist</legend>
                    <hr />
            ...
            @fragment('checklists')
                <div id="user_checklists">
                    @if(count($checklists) <= 0)
                        <div>you have no permissions for any checklists.</div>
                    @endif
                    @foreach($checklists as $checklist)
                        <div class="border-base-300 rounded-md border p-2">
                            <div class="flex items-center">
                                <span class="text-xl font-bold"> {{ $checklist->name }} </span>
                            ...
                            @if(!empty($checklist->locality))
                                <div>{{ $checklist->locality }}</div>
                            @endif

                            @if(!empty($checklist->abstract))
                                <div>{!! Purify::clean($checklist->abstract) !!}</div>
                            @endif
                        </div>
                    @endforeach
                </div>
        ...
        <x-horizontal-nav.tab name="Collections" class="flex flex-col gap-4">
            <div class="flex items-center">
                ...
                <div class="flex flex-grow justify-end">
                    @can('SUPER_ADMIN')
                        <x-button href="{{ route('collections.collmetadata.create') }}"> Create Collection </x-button>
                    @endcan
                </div>
            </div>
            <hr class="mb-4" />
            ...
            @if(count($collections) <= 0)
                <div>You have no permissions for any collections.</div>
            @endif

            @foreach($collections as $collection)
                <div class="border-base-300 relative flex items-center gap-4 rounded-md border p-4">
                    <img class="mx-auto w-16 flex-shrink" src="{{ $collection->icon }}" />
                    <div class="flex-grow">
                        ...
                        <div class="flex gap-2">
                            @foreach($roles as $role)
                                <div class="bg-base-300 w-fit rounded-full px-2">
        ...
        <x-horizontal-nav.tab name="Datasets" class="flex flex-col gap-4" x-data="{ show_create_form: false }">
            <div class="flex items-center">
                ...
                <div class="flex flex-grow justify-end">
                    <x-button @click="show_create_form = true"> Create Dataset </x-button>
                </div>
            </div>
            <hr class="mb-4" />
            ...
            <form x-show="show_create_form" hx-post="{{ url('user/profile/dataset') }}">
                @csrf
                <fieldset class="flex flex-col gap-4">
                    <legend class="text-lg font-bold">Create New Dataset</legend>
                    <hr />
            ...
            @fragment('datasets')
                @if(count($datasets) <= 0)
                    <div>You have no permissions for any datasets.</div>
                @endif
        ...
        <x-horizontal-nav.tab name="Passwords and authentication">
            ...
            @if(session('status') == 'two-factor-authentication-confirmed')
                <div class="flex flex-col gap-4">
                    <p class="font-medium">Two factor authentication confirmed and enabled successfully.</p>
                    <p>These are the recovery codes needed to get back into the account. Keep these in a safe place losing them may lead to losing access to your account.</p>
                    <div>
            ...
            @elseif(session('status') == 'two-factor-authentication-enabled')
                <div class="flex w-80 flex-col justify-center gap-4">
                    <div class="font-medium">Please finish configuring two factor authentication below.</div>

                    <div>{!! request()->user()->twoFactorQrCodeSvg(); !!}</div>

                    <form
                        hx-post="{{ url('/user/confirmed-two-factor-authentication') }}"
                        hx-swap="outerHTML"
                        hx-target="body"
                        class="flex flex-col gap-4"
                    >
                ...
                <div id="tokens-container" class="flex flex-col gap-4" x-data="{ show_token_form: false }">
                    <div>
                        <div class="flex items-center gap-2 py-2">
                            <div class="flex-grow text-2xl font-bold">Personal access tokens</div>
                    ...
                    <span>
                        Tokens you have generate that can be used to access the
                        <x-link target="_blank" href="{{ url('api/documentation') }}">Symbiota API</x-link>
                    </span>

                    <form
                        x-show="show_token_form"
                        class="m-0"
                    ...
                    >
                        @csrf
                        <fieldset class="flex flex-col gap-4">
                            <legend class="text-lg font-bold">Create New Access Token</legend>
                            <hr />
                        ...
                        <x-checkbox />--}}
                            <div class="flex gap-4">
                                ...
                                <x-button type="button" variant="error" @click="show_token_form = false"
                                    >Cancel</x-button
                                >
                            </div>
                        </fieldset>
                    ...
                    @isset($created_token)
                        <div class="border-base-300 mt-4 border-t p-4">
                            Generated api key:
                            <span class="bg-base-300 rounded-md px-2 py-1">{{ $created_token }}</span>
                            <div class="text-warning mt-1 font-bold">
                                ...
                                <div class="p-4">
                                    <div class="flex items-center gap-4">
                                        <div class="flex-grow font-bold">
                                            ...
                                            @if($token->abilities)
                                                <i class="text-base opacity-50"
                                                    >- {{ implode(',', $token->abilities) }}</i
                                                >
                                            @endif
                                        </div>
                                        ...
                                        <x-button
                                            ...
                                            hx-target="#tokens-container"
                                            hx-delete="{{ url('token/delete/' . $token->id) }}"
                                            >Delete</x-button
                                        >
                                    </div>
                                    @if($token->expires_at)
                                    ...
                                    @else
                                        <div class="text-warning font-bold underline">

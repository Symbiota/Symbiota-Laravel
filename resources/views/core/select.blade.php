...
<div {{ $attributes->withoutTwMergeClasses()->twMerge("w-fit h-fit") }}>
    ...
    >
        <button
            type="button"
            id="{{ $id }}-toggle"
            aria-labeledBy={{ $labeledBy? $labeledBy: $id . '-label' }}
            x-ref="selectButton"
        ...
        >
            <span
                x-text="selectedItem ? selectedItem.title : '{{ $select_text }}'"
                class="truncate"
                >{{ $select_text }}</span
            >
            <span class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true" class="text-base-content/50 h-5 w-5"><path fill-rule="evenodd" d="M10 3a.75.75 0 01.55.24l3.25 3.5a.75.75 0 11-1.1 1.02L10 4.852 7.3 7.76a.75.75 0 01-1.1-1.02l3.25-3.5A.75.75 0 0110 3zm-3.76 9.2a.75.75 0 011.06.04l2.7 2.908 2.7-2.908a.75.75 0 111.1 1.02l-3.25 3.5a.75.75 0 01-1.1 0l-3.25-3.5a.75.75 0 01.04-1.06z" clip-rule="evenodd"></path></svg>
            </span>
            <input
                id="{{ $id }}"
                type="hidden"
        ...
        <ul
            ...
            :class="{
                'bottom-0 mb-11': selectDropdownPosition == 'top',
                'top-0 mt-9': selectDropdownPosition == 'bottom',
            }"
            class="ring-base-300 ring-opacity-5 absolute z-10 mt-1 max-h-56 w-fit overflow-auto rounded-md bg-white shadow-md ring-1 focus:outline-none"
            x-cloak
        >
            <template x-for="(item, index) in selectableItems" :key="index + '-' + item.value">
                <li
                    ...
                    :class="{
                        'bg-base-200 text-base-content': selectableItemIsActive(item),
                        '': !selectableItemIsActive(item),
                    }"
                    @mousemove="selectableItemActive = item"
                    class="text-base-content relative flex h-full cursor-default cursor-pointer items-center px-8 py-2 select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
                >
                    <svg x-show="selectedItem.value == item.value" class="text-base-content/50 absolute left-0 ml-2 h-4 w-4 stroke-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>

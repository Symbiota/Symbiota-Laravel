...
<div class="flex flex-col gap-2">
    <div class="font-bold text-2xl">
        {{ __('checklists_voucheradmin.VOUCHCONF') }}
    </div>
    <hr/>
    <p>
        {{ __('checklists_vaconflicts.EXPLAIN_PARAGRAPH') }}
    </p>
</div>

@if(count($conflicts) > 0)
<x-text-label :label="__('checklists_vaconflicts.CONFLICT_COUNT')">
    {{ count($conflicts) }}
</x-text-label>
<form method="post" class="flex flex-col gap-4">
    ...
    <table class="w-full border-seperate text-sm">
        <thead class="bg-neutral text-neutral-content ">
            ...
            <th class="p-2">
                {{ __('checklists_vaconflicts.CHECK_ID') }}
            </th>
            <th class="p-2">
                {{ __('checklists_vaconflicts.VOUCHER_SPEC') }}
            </th>
            <th class="p-2">
                {{ __('checklists_vaconflicts.CORRECTED_ID') }}
            </th>
            <th class="p-2">
                {{ __('checklists_vaconflicts.CORRECTED_ID') }}
            </th>

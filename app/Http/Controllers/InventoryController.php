...
class InventoryController extends Controller {
    ...
    public function showOneInventory($id, Request $request) {
        ...
        if (! $inventoryObj->count()) {
            $inventoryObj = ['status' => false, 'error' => 'Unable to locate inventory based on identifier'];
        }
    ...
    public function showOneInventoryTaxa($id, Request $request) {
        ...
        if ($fullCnt) {
            ...
            $result = ['status' => false, 'error' => 'Unable to locate inventory based on identifier'];
        }

        $eor = false;
        if (($offset + $limit) >= $fullCnt) {
            $eor = true;
        }
        $retObj = [
            'offset' => (int) $offset,
    ...
    protected function getClid($id) {
        if (is_numeric($id)) {
            ...
            if (! $clid) {
                $clid = Inventory::where('guid', $id)->first()->value('clid');
            }
        }

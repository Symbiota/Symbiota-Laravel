...
<x-margin-layout x-data="{ mode: {{ $mode }} }">
    ...
    />
    <x-page-title> {{ __('traitattr_occurattributes.OCC_ATTRIBUTE_BATCH_EDIT') }} </x-page-title>

    <p class="font-bold">{{ __('traitattr_occurattributes.SELECT_UNSCORED_IMAGE_TRAIT') }}</p>
    ...
    <div @cloak($mode !== $EDIT) x-show="mode === {{ $EDIT }}">
        <x-accordion :label="__('misc_sharedterms.FILTER')" :open="true">
            <fieldset>
                <legend class="text-lg font-bold">{{ __('misc_sharedterms.FILTER') }}</legend>
                <form
                    hx-post="{{ url()->current() }}"
    ...
    @if($canReview)
        <div @cloak($mode !== $REVIEW) x-show="mode === {{ $REVIEW }}">
            <x-accordion :label="__('traitattr_occurattributes.REVIEWER')" :open="true">
                <fieldset>
                    <legend class="text-lg font-bold">{{ __('traitattr_occurattributes.REVIEWER') }}</legend>
                    <form
                        hx-post="{{ url()->current() }}"
                        hx-target="#trait-image-form"
                        hx-swap="outerHTML"
                    ...
                    >
                        ...
                        <div class="flex flex-wrap gap-4">
                            <x-select
                                class="w-auto min-w-60 flex-grow"
                                id="reviewuid"
                                default="0"
                                :items="$editorItems"
                            />

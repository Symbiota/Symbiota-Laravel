...
<div {{ $attributes->withoutTwMergeClasses()->twMerge('w-fit h-fit') }}>
    @if ($label ?? false)
        <label class="text" id="{{ $id }}-label"
            for="{{ $id }}-toggle">{{ $label }}</label>
    @endif
    <div x-data="{
        selectOpen: false,
        ...
        updateInputValue() {
            if (this.selectedItem) {
                ...
                if (input._x_model) {
                    input._x_model.set(this.selectedItem.value);
                }
    
                // Dispatch custom event for Alpine components
                $el.dispatchEvent(new CustomEvent('select-changed', {
                    detail: { value: this.selectedItem.value, item: this.selectedItem },
                    bubbles: true
                }));
            }
        },
    ...
    $el.querySelector('input').addEventListener('change', function(e) {
        ...
        <button type="button" id="{{ $id }}-toggle"
            ...
            :class="{
                'focus:ring-2 focus:ring-offset-2 focus:ring-accent hover:bg-base-200':
                    !selectOpen
            }"
            ...
            <span
                ...
                class="truncate">{{ $select_text }}</span>
            <span
            ...
            <input id="{{ $id }}" type="hidden"
                name="{{ $name ?? $id }}" x-on:change="{{ $onChange ?? '' }}"
                @bind(id) @bind(name) />
        </button>

        <ul x-show="selectOpen" x-ref="selectableItemsList"
            ...
            <template x-for="(item, index) in selectableItems"
                :key="index + '-' + item.value">
                <li @click="selectedItem=item; updateInputValue(); selectOpen=false; $refs.selectButton.focus();"
                    :id="item.value + '-' + selectId"
                    :data-disabled="item.disabled"
                    :class="{
                        'bg-base-200 text-base-content': selectableItemIsActive(
                            item),
                        '': !selectableItemIsActive(item)
                    }"
                    ...
                    <svg x-show="selectedItem.value==item.value"
                        class="absolute left-0 w-4 h-4 ml-2 stroke-current text-base-content/50"

...
<x-margin-layout>
    ...
    <div class="flex items-center">
        ...
        <div class="flex flex-grow justify-end gap-4">
            <x-button href="{{ url('projects/' . $project->pid) }}"> {{ __('Public View') }} </x-button>
        </div>
    </div>
    ...
    <x-tabs :active="0" :tabs="[__('projects.METADATA'), __('projects.INVMANAG'), __('projects.CHECKMANAG')]">
        <div class="flex flex-col gap-4">
            <fieldset class="flex flex-col gap-4">
                <div>
                    <legend class="text-primary text-2xl font-bold">{{ __('projects.EDIT') }}</legend>
                    <hr />
                </div>
                <form
                    method="post"
                    action="{{ url('projects/' . $project->pid . '/edit') }}"
            ...
            <fieldset class="flex flex-col gap-4">
                <div>
                    <legend class="text-primary text-2xl font-bold">{{ __('projects.DELPROJECT') }}</legend>
                    <hr />
                </div>
                @fragment('project_delete_form')
                    <form
                        id="project_delete_form"
                        hx-delete="{{ url('projects/' . $project->pid . '/edit') }}"
                        hx-confirm="{{ __('projects.CONFIRMDEL') }}"
                        class="flex flex-col gap-4"
                    >
                        <x-button
                            :disabled="$project->managers || count($checklists) > 0"
                            variant="error"
                            >{{ __('projects.SUBMITDELETE') }}</x-button
                        >

                        @csrf
                        @if($project->managers || count($checklists) > 0)
                            <div class="bg-warning text-warning-content rounded-md p-2">
                                @if($project->managers)
        ...
        <div id="inventory_managers" class="flex flex-col gap-4" x-cloak>
            @fragment('managers')
                <fieldset class="flex flex-col gap-4">
                    <div>
                        <legend class="text-primary text-2xl font-bold">
                            {{ __('projects.INVENTORY_PROJECT_MANAGERS') }}
                        </legend>
                        <hr />
                    </div>
                    @csrf
                    ...
                    <div class="flex flex-col gap-2">
                        @foreach($managerArr as $uid => $name)
                            <div class="bg-base-200 border-base-300 flex items-center border p-2">
                                <span>{{ $name }}</span>
                                <span class="flex flex-grow justify-end">
                                    <x-icons.delete
                                        hx-delete="{{ url('projects/' . $project->pid . '/managers/' . $uid) }}"
                                        hx-target="#inventory_managers"
                ...
                <fieldset class="flex flex-col gap-4">
                    <div>
                        <legend class="text-primary text-2xl font-bold">{{ __('projects.ADD_NEW_MANAGER') }}</legend>
                        <hr />
                    </div>
                    <form
                        hx-post="{{ url('projects/' . $project->pid . '/managers') }}"
                        class="flex flex-col gap-4"
                        hx-target="#inventory_managers"
                    >
                        @csrf
        ...
        <div id="inventory_checklists" class="flex flex-col gap-4" x-cloak>
            @fragment('checklists')
                <fieldset class="flex flex-col gap-4">
                    <div>
                        <legend class="text-primary text-2xl font-bold">{{ __('projects.ADD_A_CHECKLIST') }}</legend>
                        <hr />
                    </div>
                    <form
                        class="flex flex-col gap-4"
                    ...
                    >
                        ...
                        <x-select
                            class="w-full"
                            :label="__('projects.SELECT_CHECKLIST_TO_ADD')"
                            id="clid"
                ...
                <fieldset class="flex flex-col gap-4">
                    <div>
                        <legend class="text-primary text-2xl font-bold">{{ __('projects.DELETE_A_CHECKLIST') }}</legend>
                        <hr />
                    </div>
                    <div class="flex flex-col gap-2">
                        @foreach($checklists as $checklist)
                            <div class="bg-base-200 border-base-300 flex items-center border p-2">
                                ...
                                <span class="flex flex-grow justify-end">
                                    @csrf
                                    <x-icons.delete
                                        hx-delete="{{ url('projects/' . $project->pid . '/checklists/' . $checklist->clid) }}"
                                        hx-target="#inventory_checklists"
                                        hx-include="input[name='_token']"
                                    />
                                </span>

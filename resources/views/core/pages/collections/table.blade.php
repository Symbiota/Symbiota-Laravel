...
@endphp
<x-layout
    class="relative h-[100vh] p-0"
    x-data="{ menu_open: false }"
    :hasFooter="false"
    :hasHeader="false"
    :hasNavbar="false"
>
    <div class="relative flex h-[7rem] flex-col gap-2 px-4 pt-4">
        @if(!empty($collection))
            <x-breadcrumbs
                :items="[
            ['title' => 'Home', 'href' => url('')],
        ...
        @else
            <div class="text-primary text-2xl font-bold">Specimen Records Table</div>
        @endif
    ...
    <div
        class="w-[50%vw ]min-w-[40rem] bg-base-100 absolute top-0 left-0 z-[100] h-screen"
        x-cloak
        x-show="menu_open === 'batch_update'"
    >
        <button
            x-on:click="menu_open = false"
            class="ring-accent float-right mt-2 mr-2 h-6 w-6 rounded-md hover:ring-4 focus:ring-4 focus:outline-none"
        >
            <i class="fa fas fa-close cursor-pointer"></i>
        </button>
        <fieldset class="flex flex-col gap-4 p-4">
            ...
            <form
                ...
                x-on:htmx:after-request.window="menu_open = false"
                hx-swap="outerHTML"
                class="flex flex-col gap-4"
                hx-include="#search_form input"
            >
            ...
            >
                ...
                <div x-data="{ data: {{ json_encode($custom_fields) }} }" class="flex flex-col gap-4">
                    <template x-for="(d, index) of data" :key="'custom-form-' + index">
                        <div>
                            <div>Custom Field <span x-text="index + 1"></span></div>
                        ...
                        --}}
                                <x-occurrence-attribute-select
                                    x-bind:name="'q_customfield' + (index + 1)"
                                    class="min-w-72"
                                    :select_text="'Select Field Name'"
                                />
                                <x-select
                                    class="min-w-72"
                                    defaultValue="d.type"
                                    onChange="data[index].type = $event.target.value"
                                    x-bind:name="'q_customtype' + (index + 1)"
                                    x-bind:id="'q_customtype' + (index + 1)"
                                    :items="[
                            ...
                            ['title'=> 'IS NOT NULL', 'value' => 'NOT_NULL', 'disabled' => false ],
                                ...
                                />
                                <x-input
                                    x-bind:name="'q_customvalue' + (index + 1)"
                                    x-bind:value="data && data[index] ? data[index].value : ''"
                                    x-on:change="data[index].value = $event.target.value"
                                />
                                {{--
                    ...
                    <x-button
                        type="button"
                        @click="if (data.length < 10) data.push({ type: 'EQUALS', field: null, value: '' });"
                    >
                ...
                <div class="flex items-center gap-4">
                    ...
                    <x-button variant="neutral" onclick="document.getElementById('search_form').reset()"
                        >Display Reset Form</x-button
                    >
                </div>
            </form>
        </fieldset>
    ...
    <div
        class="relative"
        x-data="{ occid: null, column: null, column_property: null, column_value: null, loading: false }"
    >
        <div
            x-show="loading"
            x-cloak
            id="table-loader"
            class="stroke-accent absolute z-[100] flex h-full w-full justify-center"
        >
            <x-icons.loading />
        </div>
        <x-context-menu> <x-slot:menu>
    </div>
    <x-context-menu-item x-on:click="menu_open = true"> Adjust Search </x-context-menu-item>
    ...
    <x-context-menu-item>
        <a target="_blank" x-bind:href="'{{ legacy_url('/collections/individual/index.php') }}?occid=' + occid"
            >Open Occurrence in New Tab</a
        >
    </x-context-menu-item>
    ...
    <x-context-menu-item>
        <a x-bind:href="document.getElementById('occid-link-' + occid)">Open Occurrence Editor</a>
    </x-context-menu-item>
    <x-context-menu-item>
        <a target="_blank" x-bind:href="document.getElementById('occid-link-' + occid)"
            >Open Occurrence Editor in New Tab</a
        >
    </x-context-menu-item>
    ...
    <x-context-menu-item
        hx-include="#sort"
        hx-get="{{ url('collections/table') . '?fragment=table&collid='. request('collid') }}"
        hx-trigger="click"
        hx-target="#table-container"
        hx-swap="outerHTML"
    ...
    @fragment('table')
        <div id="table-container" class="relative h-[calc(100vh-7rem)] w-screen overflow-x-scroll overflow-y-scroll">
            <table class="border-seperate w-full">
                <thead class="text-left">
                    <tr class="bg-base-100">
                        @foreach($property_display_map as $property)
                            <th
                                @class([ 'bg-neutral text-neutral-content sticky top-0 text-nowrap p-2 border border-base-content z-50'
                                , 'pl-4'=> $loop->first,
                                'pr-4' => $loop->last,
                                ])
                            >
                ...
                <tbody>
                    @fragment('rows')
                        @foreach($occurrences as $occurrence)
                            <tr
                                @contextmenu="occid = {{ $occurrence->occid }}"
                                @class([ 'bg-base-200'=> $loop->even,
                            'bg-base-300' => $loop->odd,
                            ])
                            >
                                @foreach($property_display_map as $property)
                                    <td
                                        @contextmenu="column = '{{ $property['label'] }}'; column_property = '{{ $property['name'] }}'; column_value = $el.innerHTML.trim();"
                                ...
                                @endphp
                                            <a
                                                id="occid-link-{{ $occurrence->occid }}"
                                                href="{{ $url }}"
                                                class="cursor-pointer"
                                                >{{ $occurrence->{$property['name']} }}</a
                                            >
                        ...
                        @if(count($occurrences) === 100)
                            <tr
                                class="h-0 w-full"
                                hx-get="{{ url('/collections/table') . '?&fragment=rows&page='. $page + 1 . '&' . http_build_query(request()->except(['page', 'fragment'])) }}"
            ...
            <div id="scroll-loader" class="htmx-indicator">Loading more records...</div>
        </div>
    @endfragment
    </x-context-menu>
    </div>

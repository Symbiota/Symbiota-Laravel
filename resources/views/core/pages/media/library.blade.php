<x-layout class="m-auto flex flex-col gap-4 sm:w-[90%] lg:w-[70%]">
    <x-breadcrumbs
        :items="[
            ['title' => 'Home', 'href' => '/'],
            ['title' => 'Media Library' ]
    ...
    <p>This page provides a complete list of taxa that have media. Use the controls below to browse and search for media by family, genus, or species.</p>
    ...
    @fragment('taxa_list')
        <div id="taxa_list">
            @if(!empty(request('taxa')))
                <h2 class="text-2xl font-bold">Select a species to access available media</h2>
            @elseif(request('target') === 'genus')
                <h2 class="text-2xl font-bold">Select a genus to see species list</h2>
            @else
            ...
            @foreach($taxa as $taxon)
                <div>
                    @if(!request('target') && !request('taxa'))
                        <x-link
                            href="{{ url()->current() }}?taxa={{ $taxon->name }}&target=genus"
                            >{{ strtoupper($taxon->name) }}</x-link
                        >

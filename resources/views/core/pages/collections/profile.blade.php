...
<x-margin-layout x-data="{ showStatsMessages: false }">
    ...
    <div class="flex items-center gap-4">
        ...
        @endisset
        <span class="text-xl font-bold md:text-4xl">{{ $collection->collectionName }}</span>
    </div>

    <div class="flex flex-wrap items-center gap-2">
        @if($datasetKey = $collection->aggKeysStr['datasetKey'] ?? false)
            <div class="ring-accent rounded-md hover:ring-4 focus:ring-4">
                <iframe
                    title="GBIF citation"
            ...
            <a
                href="https://bionomia.net/dataset/{{ $datasetKey }}"
                class="ring-accent rounded-md hover:ring-4 focus:ring-4"
            >
                <img
                    src="https://api.bionomia.net/dataset/{{ $datasetKey }}/badge.svg"
                ...
                <x-slot name="body">
                    ...
                    >
                        ...
                        <div class="flex items-center">
                            ...
                            <div
                                id="quick-search-loader"
                                class="stroke-accent htmx-indicator flex h-6 w-6 justify-center"
                            >
                                <x-icons.loading />
                            </div>
                        </div>
    ...
    @can('COLL_EDIT', $collection->collID)
        <x-accordion :label="__('misc_collprofiles.TOGGLE_MAN')" :open="true" variant="clear-primary">
            ...
            @endphp
                ...
                <div class="flex-grow">
                    <div class="text-xl font-bold">{{ __('misc_collprofiles.DAT_EDIT') }}</div>
                    <x-list-of-links :links="$data_links" />

                    @if($collection->isTraitCodingActivated() && $isSpecimens)
                        <div class="text-lg font-bold">{{ __('misc_collprofiles.TRAIT_CODING_TOOLS') }}</div>
                ...
                @can('COLL_ADMIN', $collection->collID)
                    ...
                    <div class="flex-grow">
                        <div class="text-xl font-bold">{{ __('misc_collprofiles.ADMIN_CONTROL') }}</div>
                        <x-list-of-links :links="$admin_links" />

                        <div class="flex items-center gap-2">
                            <div class="text-lg font-bold">{{ __('misc_collprofiles.IMPORT_SPECIMEN') }}</div>
                        ...
                        <div class="text-lg font-bold">{{ __('misc_collprofiles.MAINTENANCE_TASKS') }}</div>
    ...
    <div x-show="showStatsMessages" class="flex flex-col gap-4">
        <div>
            <div class="flex items-center gap-2 text-xl font-bold">
                ...
                <div id="stats-loader" class="htmx-indicator stroke-accent h-7 w-7">
                    <x-icons.loading />
                </div>
            </div>
            <hr class="mt-2" />
        ...
        <div id="stats-output" class="bg-base-200 p-4"></div>
        <hr />
    </div>
    ...
    <div class="flex flex-col gap-2">
                    ...
                    @foreach($resourceArr as $rArr)
                        <div>
                            <x-link
                                href="{{ $rArr['url'] }}"
                                target="_blank"
                                >{{ $rArr['title'][App::currentLocale()] ?? __('misc_collprofiles.HOMEPAGE') }}</x-link
                            >
                        </div>
                    @endforeach
                </div>
        ...
        @if(($collection->publishToGbif ?? false) && ($collection->aggKeysStr['datasetKey'] ?? false))
            ...
            <x-text-label :label="__('misc_collprofiles.GBIF_DATASET')">
                <x-link :href="$gbifUrl" target="_blank" rel="noopener noreferrer"> {{ $gbifUrl }} </x-link>
            </x-text-label>
        @endif
        ...
        @if(($collection->publishToIdigbio ?? false) && ($collection->aggKeysStr['idigbioKey'] ?? false))
            ...
            <x-text-label :label="__('misc_collprofiles.IDIGBIO_DATASET')">
                <x-link :href="$idigBioUrl" target="_blank" rel="noopener noreferrer"> {{ $idigBioUrl }} </x-link>
            </x-text-label>
        @endif
...
implode(' ', [
            ...
            @if($ref_cnt = $stats->references())
                <li class="list-disc">{{ number_format($ref_cnt) }} {{ __('misc_collprofiles.PUB_REFS') }}</li>
            @endif
            ...
            <li class="list-disc lowercase">
                {{ number_format($stats->familycnt ?? 0) }} {{ __('checklists_checklist.FAMILIES') }}
            </li>
            <li class="list-disc lowercase">
                {{ number_format($stats->genuscnt ?? 0) }} {{ __('checklists_checklist.GENERA') }}
            </li>
            <li class="list-disc lowercase">
                {{ number_format($stats->speciescnt ?? 0) }} {{ __('checklists_checklist.SPECIES') }}
            </li>
            @if($total_taxa_cnt = $stats->total_taxa())
                <li class="list-disc">
                    {{ number_format($total_taxa_cnt) }} {{ __('misc_collprofiles.TOTAL_TAXA_INCLUDING') }}
                </li>
            @endif
        </ul>
    </div>
    ...
    <x-accordion :label="__('checklists_dynamicmap.MORE_DETAILS')" :open="true" variant="clear-primary">
        <x-text-label :label="__('misc_collprofiles.COLLECTION_TYPE')"> {{ $collection->collType }} </x-text-label>
        ...
        @if($collection->managementType != 'Live Data')
            <x-text-label :label="__('misc_collprofiles.LAST_UPDATE')"> {{ $stats->datelastmodified }} </x-text-label>
        @endif
        ...
        @if($collection->managementType == 'Snapshot')
            <x-text-label :label="__('misc_collprofiles.IPT_SOURCE')">
                @foreach($collection->dwcaPaths() as $path)
                    <x-link href="{{ $path->path }}">{{ $path->title }}</x-link>
                    @if(!$loop->last) {{ '|' }}@endif
                @endforeach
            </x-text-label>
        @endif
        ...
        @if($collection->rights)
            <x-text-label :label="__('misc_collmetadata.LICENSE')">
                <div class="w-32">{!! Purify::clean(GeneralUtil::getRightsHtml($collection->rights)) !!}</div>
            </x-text-label>
        @else
            <div>
                <x-link
                    href="{{ url('usagepolicy') }}"
                    target="_blank"
                    >{{ __('misc_collprofiles.USAGE_POLICY') }}</x-link
                >
            </div>
        @endif

        <x-text-label :label="__('individual.RIGHTS_HOLDER')"> {{ $collection->rightsHolder }} </x-text-label>

        <x-text-label :label="__('individual.ACCESS_RIGHTS')"> {{ $collection->accessRights }} </x-text-label>
    </x-accordion>
            ...
            <x-tabs :tabs="$stats_tabs" class:body="border-x-0 border-b-0">
                @isset($stats->dynamicProperties->families)
                    <x-chart
                        ...
                        type="bar"
                        width="{{ calc_chart_width(count($fam_georef_stats)) }}"
                        height="600"
                        class="w-full pb-4"
                        :values="$fam_georef_stats"
                    />
                @endisset
                ...
                @isset($stats->dynamicProperties->countries)
                    <x-chart
                        ...
                        type="bar"
                        width="{{ calc_chart_width(count($country_georef_stats)) }}"
                        height="600"
                        class="w-full pb-4"
                        :values="$country_georef_stats"
                    />
                @endisset
            </x-tabs>
        </div>

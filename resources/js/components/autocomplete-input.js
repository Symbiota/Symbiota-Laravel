function autoSearchInit(el) {
    ...
    const request_settings = JSON.parse(
        input.getAttribute("data-request-config"),
    );
    ...
    menu.addEventListener("htmx:after-swap", (e) => {
        ...
        for (let i = 0; i < e.target.children.length; i++) {
            ...
            if (
                ...
                menu.children[i].innerHTML.toLowerCase()
            ) {
                input.dispatchEvent(
                    new CustomEvent("auto_input_select", {
                        detail: { selection: menu.children[i] },
                    }),
                );
            }

            e.target.children[i].classList.add("cursor-pointer");
            e.target.children[i].addEventListener("mouseover", () =>
                setIndex(i),
            );
            e.target.children[i].addEventListener("mousedown", (event) => {
                event.preventDefault();
                const incoming_option = menu.children[i].innerHTML;
                const prev_options = input.value.slice(
                    0,
                    input.value.lastIndexOf(",") + 1,
                );
                input.value =
                    (prev_options ? prev_options + " " : prev_options) +
                    incoming_option;
    ...
    input.addEventListener("htmx:configRequest", (e) => {
        e.detail.parameters[input_name] = getLastValue(
            e.detail.parameters[input_name],
        );
            ...
            if (request_settings.alias) {
                for (let alias in request_settings.alias) {
                    if (e.detail.parameters[alias]) {
                        ...
                        e.detail.parameters[request_settings.alias[alias]] =
                            aliased_value;
                    }
                }
            }
    ...
    input.addEventListener("keydown", (e) => {
        switch (e.key) {
            ...
            case "Enter":
                ...
                input.dispatchEvent(
                    new CustomEvent("auto_input_select", {
                        detail: { selection: menu.children[getIndex()] },
                    }),
                );
                break;
        }

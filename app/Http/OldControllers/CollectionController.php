...
class CollectionController extends Controller {
    ...
    public function showAllCollections(Request $request) {
        ...
        if ($request->has('managementType')) {
            if ($request->managementType == 'live') {
                $conditions[] = ['managementType', 'Live Data'];
            } elseif ($request->managementType == 'snapshot') {
                $conditions[] = ['managementType', 'Snapshot'];
            } elseif ($request->managementType == 'aggregate') {
                $conditions[] = ['managementType', 'Aggregate'];
            }
        }
        if ($request->has('collectionType')) {
            if ($request->collectionType == 'specimens') {
                $conditions[] = ['collType', 'Preserved Specimens'];
            } elseif ($request->collectionType == 'observations') {
                $conditions[] = ['collType', 'Observations'];
            } elseif ($request->collectionType == 'researchObservations') {
                $conditions[] = ['collType', 'General Observations'];
            }
        }
    ...
    public function showOneCollection($id, Request $request) {
        $collectionObj = null;
        if (is_numeric($id)) {
            $collectionObj = Collection::find($id);
        } else {
            $collectionObj = Collection::where('collectionGuid', $id)->first();
        }
        if (! $collectionObj->count()) {
            $collectionObj = ['status' => false, 'error' => 'Unable to locate collection based on identifier'];
        }

        return response($collectionObj);
    }

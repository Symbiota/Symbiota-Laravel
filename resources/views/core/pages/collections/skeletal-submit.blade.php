...
>
    ...
    />
    <div>
        <h1 class="font-sans text-4xl font-bold">{{ __('editor_skeletalsubmit.OCC_SKEL_SUBMIT') }}</h1>
        <h2 class="font-sans text-2xl font-bold">{{ $collection->collectionName }}</h2>
    </div>

    <div>
        <div class="flex items-center gap-2">
            <div class="text-2xl font-bold">{{ __('editor_skeletalsubmit.SKELETAL_DATA') }}</div>
            <x-button
                class="h-5 w-5 justify-center rounded-full"
            ...
            <x-text-label class="flex flex-grow justify-end" :label="__('editor_skeletalsubmit.SESSION')">
    ...
    <div x-show="show_description" class="flex flex-col gap-2">
        <p>{{ __('editor_skeletalsubmit.SKELETAL_DESCIPRTION_1') }}</p>
        <p>{{ __('editor_skeletalsubmit.SKELETAL_DESCIPRTION_2') }}</p>
        <p>{{ __('editor_skeletalsubmit.SKELETAL_DESCIPRTION_3') }}</p>
    </div>
        ...
        <div class="flex flex-col gap-2">
            @foreach([
                ...
                <x-checkbox
                    ...
                    x-bind:checked="{{ $checkbox['show'] }}"
                    x-on:change="{{ $checkbox['show'] }}=$event.target.checked"
                />
            @endforeach
            <x-radio
                :label="__('editor_skeletalsubmit.CATNUM_MATCH')"
                name="addaction"
                :options="[
    ...
    @fragment('skeletal-submit')
        ...
        >
            ...
            <x-input
                ...
                :label="__('editor_skeletalsubmit.AUTHORSHIP')"
                x-show="show_authorship"
            />
            @can('TAXONOMY')
                <x-button :href="url('taxon/create')"> {{ __('sitemap.ADDTAXANAME') }} </x-button>
            @endcan
            <x-input name="family" :label="__('taxa.FAMILY')" x-show="show_family" />

            <x-checkbox
                name="recordsecurity"
                :label="__('editor_skeletalsubmit.PROTECT_LOCALITY_DETAILS_FROM_PUBLIC')"
                x-show="show_security"
            />
            ...
            <div class="flex gap-2" x-show="show_collector || show_collector_number || show_collector_date">
                ...
                <x-input
                    name="recordnumber"
                    :label="__('editor_skeletalsubmit.COLLECTOR_NO')"
                    x-show="show_collector_number"
                />
                <x-input
                    name="eventdate"
                    :label="__('editor_skeletalsubmit.COLLECTION_DATE')"
                    x-show="show_collector_date"
                />
            </div>
            ...
            <div class="flex gap-2" x-show="show_label_project || show_processing_staus || show_language">
                <x-input
                    name="labelproject"
                    :label="__('editor_skeletalsubmit.LABEL_PROJECT')"
                    x-show="show_label_project"
                />
                <x-select
                    default="0"
                    class="w-full"
            ...
            <div class="flex gap-2">
                ...
                <x-input
                    ...
                    :label="__('cleaning.OTHER_CAT_NUMS')"
                    x-show="show_other_catalognumbers"
                />
            </div>

            <div class="flex gap-2">
                <x-button> {{ __('profile_occurrencemenu.ADD_RECORD') }} </x-button>

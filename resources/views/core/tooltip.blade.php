...
>
    <span
        ...
        x-cloak
    >
        <div x-show="tooltipVisible" x-transition class="relative rounded bg-black/90 px-2 py-1 text-white">
            <p x-text="tooltipText" class="block flex-shrink-0 text-base whitespace-nowrap"></p>
            <div
                x-ref="tooltipArrow"
                x-show="tooltipArrow"
                :class="{
            ...
            >
                <div
                    ...
                    class="h-1.5 w-1.5 transform bg-black/90"
                ></div>
            </div>
        </div>
    </span>

    <span x-ref="content" class="w-fit cursor-pointer rounded-full">{{ $slot }}</span>
</div>

        ...
        @foreach($collections as $specimen)
            ...
            @endphp
                @if(is_numeric($media_count) && intval($media_count) > 0)
                    <div>
                        <x-link
                            href="{{ url('media/search') }}?collId={{ $specimen->collId }}"
                            >{{ $specimen->collectionName }}</x-link
                        >
        ...
        @foreach($collections as $observation)
            ...
            @endphp
                @if(is_numeric($media_count) && intval($media_count) > 0)
                    <div>
                        <x-link
                            href="{{ url('media/search') }}?collId={{ $observation->collId }}"
                            >{{ $observation->collectionName }}</x-link
                        >
                        ({{ number_format($media_count) }})
                    </div>
                @endif

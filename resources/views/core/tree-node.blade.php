        ...
        <li x-data="{ open: false }">
            @if(count($node->children ?? []) > 0)
                <div style="padding-left: {{ ($rankMap[$node->rankID]/$standardizingFraction ?? 0) }}rem" @click="open = !open">
                    <x-link class="no-underline">{{ $node->sciName }} {{$node->rankID}} [depth is {{$depth}}] (Click to <span x-text="open ? 'collapse' : 'show all direct children'"></span>)</x-link>
                </div>
            ...
            @else
                <div style="padding-left: {{ $node->rankID > $parentRankId ? ($rankMap[$node->rankID]/$standardizingFraction + $depth ?? 0) : 0 }}rem">
                    <x-link href="{{url('taxon/' . $node->tid)}}">{{ $node->sciName }} {{$node->rankID}} [depth is {{$depth}}] [parent rankID is {{$parentRankId}}]</x-link>
                </div>
            @endif
        </li>
    @endforeach

...
@endphp
        @if(!empty($collection))
            <alternateIdentifier> {{ url('collections/' . $collection->collID) }} </alternateIdentifier>
        @endif
        <title xml:lang="{{ $lang }}">
            @if(!empty($collection))
        ...
        <intellectualRights>
            <para
                >To the extent possible under law, the publisher has waived all rights to these data and has dedicated
                them to the
                <ulink url="http://creativecommons.org/licenses/by-nc/4.0/">
                    <citetitle></citetitle>
                </ulink>
            </para>
        </intellectualRights>
    ...
    <additionalMetadata>
        <metadata>
            <symbiota id="">
                ...
                <citation identifier="{{ $identifier }}"> {{ config('app.name') }}-{{ $identifier }} </citation>
                <physical>

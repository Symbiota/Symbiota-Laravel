...
<x-margin-layout :hasHeader="false" :hasFooter="false" :hasNavbar="false" x-data="{editOpen: {{ $eMode }}}">
    ...
    <div class="flex items-center gap-4">
	    ...
	    <h1 class="text-4xl text-primary font-bold">
            {{ __('imagelib_imgdetails.MEDIA_DETAILS') }}
        </h1>
        @if($imgArr)
            ...
            @if($imgArr['occid'])
				<div title="{{ __('imagelib_imgdetails.EDITING_PRIVILEGES') }}">
					<x-link class="flex items-center gap-1" href="{{ legacy_url('collections/editor/occurrenceeditor.php?&tabtarget=2&occid=' . $imgArr['occid']) }}" target="_blank">
                        ...
                        <span class="text-sm">
                            {{ __('imagelib_imgdetails.SPEC') }}
                        </span>
					</x-link>
				</div>
            @elseif($isEditor)
                <div>
                    <x-link class="flex items-center gap-1" x-bind:href="'#emode=' + (editOpen? 'true':'false')" @click="editOpen=!editOpen" title="{{ __('imagelib_imgdetails.EDIT_MEDIA') }}">
                        <img src="{{ legacy_url('images/edit.png') }}" class="w-5"/>
                        <span class="text-sm">
                            {{ __('imagelib_imgdetails.IMG') }}
                        </span>
                    </x-link>
                </div>
            @endif
    ...
    @else
        @if($isEditor && !$imgArr['occid'])
            <div x-show="editOpen" x-cloak id="imageedit" class="flex flex-col gap-4 ">
                <form name="editform"  method="post" target="_self" class="p-4 border border-base-300">
                    @csrf
                    <x-formgroup :label="__('imagelib_imgdetails.EDIT_IMAGE_DETAILS')">
                        <x-input :label="__('imagelib_imgdetails.CAPTION')" class="w-65" :inline="true"
                            name="caption" value="{{ $imgArr['caption'] }}"
                        />

                        <div class="flex flex-wrap items-center gap-1">
                            <label for="creatorUid" class="font-bold">{{ __('imagelib_imgdetails.CREATOR_USER_ID') }}:</label>
                            ...
                            <span>
                                * {{ __('imagelib_imgdetails.USER_REGISTERED_SYSTEM') }}
                            </span>
                        </div>

                        <x-input :label="__('imagelib_imgdetails.CREATOR_OVERRIDE')" class="max-w-65" :inline="true"
                            name="creator" value="{{ $imgArr['creator'] }}"
                            :assistive_text="'* ' . __('imagelib_imgdetails.OVERRIDE_SELECTION')"
                        />

                        <x-input :label="__('imagelib_imgdetails.MANAGER')" class="max-w-100" :inline="true"
                            name="owner" value="{{ $imgArr['owner'] }}"
                        />

                        <x-input :label="__('imagelib_imgdetails.SOURCE_URL')" class="max-w-115" :inline="true"
                            name="sourceUrl" value="{{ $imgArr['sourceUrl'] }}"
                        />

                        <x-input :label="__('imagelib_imgdetails.COPYRIGHT')" class="max-w-115" :inline="true"
                            name="copyright" value="{{ $imgArr['copyright'] }}"
                        />

                        <x-input :label="__('imagelib_imgdetails.RIGHTS')" class="max-w-115" :inline="true"
                            name="rights" value="{{ $imgArr['rights'] }}"
                        />

                        <x-input :label="__('imagelib_imgdetails.LOCALITY')" class="max-w-138" :inline="true"
                            name="locality" value="{{ $imgArr['locality'] }}"
                        />

                        <div class="flex items-center gap-2">
                            <x-input :label="__('imagelib_imgdetails.OCCURRENCE_RECORD') . ' #'" :inline="true"
                        ...
                        <x-input :label="__('imagelib_imgdetails.NOTES')" class="max-w-138" :inline="true"
                            name="notes" value="{{ $imgArr['notes'] }}"
                        />

                        <x-input :label="__('imagelib_imgdetails.SORT_SEQUENCE')" size="5" :inline="true"
                            name="sortSequence" value="{{ $imgArr['sortSequence'] ?? '' }}"
                        />

                        <div>
                            <x-input :label="__('imagelib_imgdetails.WEB_IMAGE')" :inline="true" required
                        ...
                        <div>
                            <x-input :label="__('imagelib_imgdetails.THUMBNAIL')" :inline="true"
                        ...
                        <div style="margin-top:2px;">
                            ...
                            <x-input :label="__('imagelib_imgdetails.LARGE_IMAGE')" :inline="true"
                        ...
                        <div style="margin-top:2px;">
                            <x-button type="submit" name="submitaction" id="editsubmit" value="Submit Image Edits">{{ __('imagelib_imgdetails.SUBMIT_IMAGE_EDITS') }}</x-button>
                        </div>
                    </x-formgroup>
                </form>
                <form name="changetaxonform" action="{{ url('media/' . $mediaID . '/transfer/taxa') }}" method="post" target="_self" class="p-4 border border-base-300">
                    @csrf
                    <x-formgroup :label="__('imagelib_imgdetails.TRANSFER_IMAGE_TO_DIFF_NAME')">
                        <x-taxa-search id="taxa" :label="__('imagelib_imgdetails.TRANSFER_TO_TAXON')" :hide_selector="true" :hide_synonyms_checkbox="true" />
                        ...
                        <x-button type="submit" name="submitaction" value="Transfer Image">{{ __('imagelib_imgdetails.TRANSFER_IMAGE') }}</x-button>
                    </x-formgroup>
                </form>
                <form name="deleteform" method="post" target="_self" onsubmit="return window.confirm('{{ __('imagelib_imgdetails.DELETE_IMAGE_FROM_SERVER') }}');" class="p-4 border border-base-300">
                    @csrf
                    <x-formgroup :label="__('imagelib_imgdetails.AUTHORIZED_REMOVE_IMAGE')">
                        <input type="hidden" name="_method" value="DELETE">
                        <input name="mediaid" type="hidden" value="{{ $mediaID; }}" />
                        <div style="margin-top:2px;">
                            <x-button variant="error" type="submit" name="submitaction" id="submit" value="Delete Image">{{ __('imagelib_imgdetails.DELETE_IMAGE') }}</x-button>
                        </div>
                        <input type="hidden" name="tid" value="{{ $imgArr["tid"]; }}" />
                        <x-checkbox id="removeimg" name="removeimg" :checked="false" :label="__('imagelib_imgdetails.REMOVE_IMG_FROM_SERVER')"/>
                        <div class="text-error">
                            {{ __('imagelib_imgdetails.BOX_CHECKED_IMG_DELETED') }}
                        </div>
                    </x-formgroup>
                </form>
            </div>
        ...
        <div class="flex">
            ...
            <div class="px-4">
                ...
                @else
                <div>
                    <x-link href="{{ url('usagepolicy#images') }}">{{ __('imagelib_imgdetails.COPYRIGHT_DETAILS') }}</x-link>
                </div>
                @endif
                ...
                <div>
                    <x-link href="{{ $imgUrl }}">
                        {{ __('imagelib_imgdetails.OPEN_MEDIUM_SIZED_IMAGE') }}
                    </x-link>
                </div>
                @endif
                ...
                <div>
                    <x-link href="{{ $origUrl }}">
                        {{ __('imagelib_imgdetails.OPEN_LARGE_IMAGE') }}
                    </x-link>
                </div>
                @endif
                ...
                <div style="margin-top:20px;">
                    {{ __('imagelib_imgdetails.ERROR_COMMENT_ABOUT_IMAGE') }}<br/> {{ __('imagelib_imgdetails.SEND_EMAIL') }}

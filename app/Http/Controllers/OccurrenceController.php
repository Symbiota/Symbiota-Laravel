...
class OccurrenceController extends Controller {
    ...
    public function showOneOccurrence($id, Request $request) {
        ...
        if ($occurrence) {
            if (! $occurrence->occurrenceID) {
                $occurrence->occurrenceID = $occurrence->recordID;
            }
            if ($request->input('includeMedia')) {
                $occurrence->media;
            }
            if ($request->input('includeIdentifications')) {
    ...
    public function skeletalImport(Request $request) {
        ...
        if ($user = $this->authenticate($request)) {
            ...
            $skeletalFieldsAllowed = ['catalogNumber', 'otherCatalogNumbers', 'sciname', 'scientificNameAuthorship', 'family', 'recordedBy', 'recordNumber', 'eventDate', 'eventDate2', 'country', 'stateProvince', 'county', 'processingStatus'];
            foreach ($updateArr as $fieldName => $fieldValue) {
                if (! $fieldValue) {
                    unset($updateArr[$fieldName]);
                } elseif (! in_array($fieldName, $skeletalFieldsAllowed)) {
                    unset($updateArr[$fieldName]);
                }
            }
            if (! $updateArr) {
            ...
            if ($targetOccurrence) {
                ...
                if (empty($updateArr['processingStatus'])) {
                    $updateArr['processingStatus'] = 'unprocessed';
                }
            ...
            if ($pub->direction == 'import') {
                ...
                if ($sourcePortalID && $remoteOccid) {
                    ...
                    if ($remoteOccurrence = $this->getAPIResponce($url)) {
                        ...
                        if (! $remoteOccurrence['occurrenceRemarks']) {
                            unset($remoteOccurrence['occurrenceRemarks']);
                        }
    ...
    private function isAuthorized($user, $collid) {
        foreach ($user['roles'] as $roles) {
            if ($roles['role'] == 'SuperAdmin') {
                return true;
            } elseif ($roles['role'] == 'CollAdmin' && $roles['tablePK'] == $collid) {
                return true;
            } elseif ($roles['role'] == 'CollEditor' && $roles['tablePK'] == $collid) {
                return true;
            }
        }

...
<div x-data="{ el: $el, open: false, results: {{ !$result->isEmpty() ? 'true' : 'false' }} }" x-init="autoSearchInit($el)" class="w-full">
    ...
    <div {{ $menu->attributes->twMerge('relative w-full') }}>
        <div id="menu-loader-{{ $id }}" class="htmx-indicator"
            x-show="open" x-cloak>
            <div
                {{ $indicator->attributes->twMerge('absolute w-full mt-1 bg-base-100 border-base-300 border p-1') }}>
                @if ($indicator->isEmpty())
                    <div
                        class="flex items-center justify-center gap-1 text-base-content">
        ...
        <div x-on:htmx:after-swap="open = true; results = $el.children.length > 0"

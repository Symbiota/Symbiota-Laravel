...
<x-layout class="flex flex-col gap-4" :hasHeader="false" :hasFooter="false" :hasNavbar="false">
    ...
    <p>By downloading data, the user confirms that he/she has read and agrees with the general <x-link class="text-base" href="{{ url('usagepolicy') }}">data usage terms</x-link>. Note that additional terms of use specific to the individual collections may be distributed with the data download. When present, the terms supplied by the owning institution should take precedence over the general terms posted on the website.</p>
    ...
    >
        ...
        <fieldset class="flex flex-col gap-2">
            ...
            <div class="bg-error text-error-content w-fit rounded-md p-2" x-show="!allow_extensions" x-cloak>
        ...
        <fieldset class="flex flex-col gap-2">
            ...
            <x-checkbox
                label="Compressed ZIP file"
                :checked="true"
                name="compressed"
                x-on:change="allow_extensions = !allow_extensions"
            />
        </fieldset>
